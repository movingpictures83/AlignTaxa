...
#Parameters
TAXA_OFFSET = 2 #Index offset in dataset where taxon data starts 
...
##		print CurrentSampleT
	...
	for a in np.arange(0.01, 3.01, 0.01): #This paramater needs to be adjusted according to data set properties as well as warp function type
		for b in np.arange(-10.0, 10.5, 0.5): #This paramater needs to be adjusted according to data set properties as well as warp function type
...
##	print "# of Taxa", len(taxaNames)
	...
	for line in infile:
		...
		if previousSubjectID != '' and previousSubjectID != subjectID:
			...
			currSubjectSample = {}
		weekSampleObtained = float(tokens[1])
...
#	main(['getAlignmentsIBD.py', 'human_ibd_microbiota.tsv', 'True', 'human_ibd_microbiota_taxon_pairwise_subject_ranking.tsv'])
#	main(['getAlignmentsIBD.py', 'human_ibd_microbiota.tsv', 'True', 'human_ibd_microbiota_taxon_pairwise_all_subject_ranking.tsv'])
